...
def init_params(params, silent=False):
    ...
    if wavevector_mode == 2:
        ...
        if not silent:
            print(
                'Running with k_par=%7.4g and k_perp=%7.4g.' %
                (kpar, kperp), file=outfile)


# Generate Bessel function array -- this is only done once, unless kperp
# or Tpar_Tperp change
...
# This routine manages the generation of the Bessel and Z function arrays
# for analytical vperp integrals
...
def calc_bessel_Z_analytical(j, omega, kpar, kperp, Tpar_Tperp, nb):
    ...
    if first_info[1] != kperp or first_info[2] != Tpar_Tperp:
        ...
        first_info = [kpar, kperp, Tpar_Tperp, omega]
    j_shift = j + nb + 1
    return zn_arr[j_shift + 1], zn_arr[j_shift -
                                       1], zn_arr[j_shift], ive_arr[j_shift], ive_arr[j_shift - 1]

# Compute the term with number "j" for all Bessel sums


def calc_sums_analytical(
        j,
        omega,
        kpar,
        kperp,
...
# Define some abbreviations to reduce number of necessary operations
    ...
    summand[1] = 2 * j * znp1 * gn - znp1 * \
        (bgnpr1 + bgnpr3)  # the m+1 half of I_1
    summand[2] = (1 + omjm1_znm1) * (bgnpr1 + bgnpr3)
    summand[3] = 2 * j * (1 + omjp1_znp1) * gn - \
        (1 + omjp1_znp1) * (bgnpr1 + bgnpr3)
...
def get_sums_analytical(kperp, kpar, omega, Tpar_Tperp, nb):
    ...
    # zeroth term
    j = 0
    znp1, znm1, zn, iveval, ivevalm1 = calc_bessel_Z_analytical(
        j, omega, kpar, kperp, Tpar_Tperp, nb)
    totalsum = calc_sums_analytical(
        j, omega, kpar, kperp, znp1, znm1, zn, iveval, ivevalm1)
    for j in range(1, nb + 1):
        # positive j
        znp1, znm1, zn, iveval, ivevalm1 = calc_bessel_Z_analytical(
            j, omega, kpar, kperp, Tpar_Tperp, nb)
        psummand = calc_sums_analytical(
            j, omega, kpar, kperp, znp1, znm1, zn, iveval, ivevalm1)
        totalsum += psummand
        ...
        znp1, znm1, zn, iveval, ivevalm1 = calc_bessel_Z_analytical(
            -j, omega, kpar, kperp, Tpar_Tperp, nb)
        nsummand = calc_sums_analytical(-j,
                                        omega,
                                        kpar,
                                        kperp,
                                        znp1,
        ...
        if any(np.isnan(totalsum)):
            print('WARNING: Encountered NaN in Bessel sums! Entries: ', j,
                  omega, np.isnan(totalsum), zn, znp1, znm1, file=outfile)
            exit()
        ...
        if all(abs(psummand + nsummand) / abs(totalsum) < acc) and j > 2:
            ...
            if j == nb - 1 and warned_Bessel != kperp:
                print('Warning: Could not reach desired convergence accuracy in Bessel sums! Max. deviation: %g' % (
                    max(abs(psummand + nsummand) / abs(totalsum))), file=outfile)
                warned_Bessel = kperp
...
def dispersion_relation_analytical(
    ...
    M = 1j * eta * k2 * inv_kpar + omega - 0.5 * inv_kpar * (1j * eta * kperp**2 * inv_kpar + omega) * (summand[6] - summand[5]) + 0.5 * (
        Tpar_Tperp - 1.) / Tpar_Tperp * inv_kpar * (summand[8] - summand[7]) + 1j * eta * inv_kpar * (summand[2] - summand[3])
    N = 1j / beta * k2 * inv_kperp + 1j * omega * inv_kperp * (summand[9] + inv_b * summand[10] - 0.5 * (summand[6] + 3 * summand[5])) - 1j * inv_kperp * (
        Tpar_Tperp - 1.) / Tpar_Tperp * (summand[11] + inv_b * summand[12] - 0.5 * (summand[8] + 3 * summand[7]))
    O = 0.5j * gam / tau * \
        (-inv_kperp * (summand[2] - summand[3]) + 0.5 * kperp * inv_kpar * (summand[6] - summand[5]))

    P = 1j * kpar / beta - 1j * inv_kpar * (1j * eta * kperp**2 * inv_kpar + omega) * (summand[9] + inv_b * summand[10] + 0.5 * (summand[6] - summand[5])) + 1j * inv_kpar * (
        Tpar_Tperp - 1.) / Tpar_Tperp * (summand[11] + inv_b * summand[12] + 0.5 * (summand[8] - summand[7])) + eta * inv_kpar * (summand[2] + summand[3])
    Q = -(1j * eta * k2 + omega * kpar) * inv_kperp + 0.5 * omega * inv_kperp * \
        (summand[6] - summand[5]) - 0.5 * (Tpar_Tperp - 1.) / Tpar_Tperp * inv_kperp * (summand[8] - summand[7])
    R = -0.5 * gam / tau * (inv_kperp * (summand[2] + summand[3]) + kperp * inv_kpar * (
        summand[9] + inv_b * summand[10] + 0.5 * (summand[6] - summand[5])))

    S = -(1j * eta * kperp**2 * inv_kpar + omega) * 1j * inv_kperp * inv_kpar * Tpar_Tperp * \
        (summand[0] + summand[1]) + 1j * inv_kpar * inv_kperp * (Tpar_Tperp - 1) * (summand[2] + summand[3]) + 2 * eta * kperp * inv_kpar * summand[4]
    T = -Tpar_Tperp * omega * inv_kperp**2 * \
        (summand[0] - summand[1]) + inv_kperp**2 * (Tpar_Tperp - 1) * (summand[2] - summand[3])
    U = -1 - 0.5 * gam / tau * \
        (2 * summand[4] + Tpar_Tperp * inv_kpar * (summand[0] + summand[1]))

    global mat
    mat = [[M, N, O], [P, Q, R], [S, T, U]]
    if det(mat) > 1:
        print(summand[6], -summand[5], file=outfile)
    return det(mat)
...
def DR_point(params, x):
    ...
    omega = x[0] + 1j * x[1]
    return abs(DR(omega, beta, tau, Tpar_Tperp,
                  kperp, kpar, gam, eta, nb, theta, k))
...
def DR_Solve(params, start):
    ...
    if ier:
        ...
        if abs(residual) > 1e-8:
            print(
                'WARNING: Large residual, root may not be properly converged!',
                file=outfile)
        ...
        u, v = eig(mat)
        eps = 2 * amin(abs(u))
        # will be 0 if it doesn't move
        move = abs(x[0] - start[0]) + abs(x[1] - start[1])
...
def PlotDR2d(
    params, np=(
        100, 100), cntr=[
            0, -1], raxis=3., iaxis=3., outfilename='output.log'):
    ...
    for i in range((np[0] * np[1])):
        if (i % np[0]) == 0:
            print(
                'Computing line ', int(
                    i / np[0]) + 1, 'of', np[1], file=outfile)
            outfile.flush()
        DR2d[i] = abs(DR(omega[i], beta, tau, Tpar_Tperp,
                         kperp, kpar, gam, eta, nb, theta, k))
