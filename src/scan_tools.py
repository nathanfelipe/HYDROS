...
# scandirec defines an angle along which the new starting points will be
# aligned
...
def next_start(X, F, G, i, j, scandirec=0, radius=1., method='default'):
    ...
    if 'default' in method:  # use old root position as starting point, plus potential scandirec modifications
        ...
        if i > 1:
            ...
            fval = F[i] + np.diff(F)[i - 1] / \
                np.diff(X)[i - 1] * np.diff(X)[i] + df
            gval = G[i] + np.diff(G)[i - 1] / \
                np.diff(X)[i - 1] * np.diff(X)[i] + dg
        else:
            fval = F[i]
            gval = G[i]
        if 'ldr' in method:
            gval = -0.75 * abs(G[i] + np.diff(G)[i - 1] /
                               np.diff(X)[i - 1] * np.diff(X)[i]) + dg
    # lower damping rate (designed to prefer more weakly damped modes)
    elif 'ldr' in method:
        # always aim for lower damping rate so the less damped mode will be
        # chosen
        gval = -0.75 * abs(G[i]) + dg
        # include slight frequency shift (depending on the scan direction) in
        # order to help jump across resonances
        fval = F[i] + 0.03 * F[i] * np.sign(np.diff(X)[-1]) + df
    # lower limit for damping rates, to prevent runaway to extremely small
    # values
    if abs(gval) < 1e-13:
        gval = np.sign(gval) * 1e-13
    return [fval, gval]
        ...
        while (j <= jmax):
            ...
            if err == 1 and (By is not None):  # no error
                if abs(omega) < 1e6 and abs(gamma) < 1e6:
                    ...
                    if index == 0 or abs(omega -
                                         OMEGA[index -
                                               1 -
                                               shift]) > 1e-9 or abs(gamma -
                                                                     GAMMA[index -
                                                                           1 -
                                                                           shift]) > 1e-9:
                        ...
                        d2 = 0.
                        d3 = abs(log10(abs(By)) -
                                 log10(abs(BY[index - 1 - shift])))
                        d4 = abs(log10(abs(dn)) -
                                 log10(abs(DN[index - 1 - shift])))
                        d5 = abs(np.angle(By) -
                                 np.angle(BY[index - 1 - shift]))
                        d6 = abs(np.angle(dn) -
                                 np.angle(DN[index - 1 - shift]))
                        if (all([d3, d4] <= limit) and (d1 <= limit or abs(omega) < 1e-3) and (d2 <= limit or abs(gamma)
                                                                                               < 1e-3)) and (d5 < cpl or abs(d5 - 2 * np.pi) < cpl) and (d6 < cpl or abs(d6 - 2 * np.pi) < cpl):
                            # very close to guess
                            ...
                            deltax = log10(
                                XARRAY[index - 1 - shift]) - log10(XARRAY[index - 2 - shift])
                            if abs(OMEGA[index - 2 - shift]) != 0.:
                                deltay = log10(
                                    abs(OMEGA[index - 1 - shift])) - log10(abs(OMEGA[index - 2 - shift]))
                            else:
                                deltay = log10(abs(OMEGA[index - 1 - shift]))
                            slope = deltay / deltax
                            if (abs(omega) < 1e-3 and (slope < -
                                                       2. or abs(OMEGA[index -
                                                                       1 -
                                                                       shift]) < 1e-3) and (d2 < limit or abs(gamma) < 1e-3)):
                                return [omega, gamma, By, dn, err]
                            d_arr = [
                                d1 > limit,
                                d2 > limit,
                            ...
                            if any(d_arr[0:2]):
                                print(
                                    'Discontinuity in omega/gamma, %7.4e, %7.4e' %
                                    (d1, d2), file=outfile)
                            if any(d_arr[2:4]):
                                print(
                                    'Discontinuity in amplitudes, %7.4e, %7.4e' %
                                    (d3, d4), file=outfile)
                            if any(d_arr[4:]):
                                print(
                                    'Discontinuity in cross phases, %7.4e, %7.4e' %
                                    (d5, d6), file=outfile)
                            update_output(
                                '\r Continuity problem on index %d, attempting to recover mode... ' %
                                (index + 1))
                            j += jstep
                    else:
                ...
                print(
                    "Eigenvalue smallness condition not fulfilled, or solver error",
                    file=outfile)
                j += jstep
        return None
    ...
    if scan_1 is None:
        ...
        for step in steps:
            ...
            while (scan_1 is None and scandirec < np.pi):
                ...
                # which returned the best residual, then we proceed to the next
                # probing radius.
                res = Scan(-1, 1, -1, scandirec,
                           radius=step * radius, probe=True)
